#!/bin/bash
#
# Case CLI binary installation script
# ------------------------------------
#
...
CYN  "🍬 Case CLI binary installation script"
echo "---------------------------------------"
echo ""
...
case "$PROCESSOR" in
    arm* | aarch* | ppc* )
        if [ "$OS_FLAVOUR" != Darwin ]; then
            ...
            echo "  => $(CYN https://github.com/metaplex-foundation/case)"
            echo ""
...
REMOTE="https://github.com/metaplex-foundation/case/releases/latest/download/"
...
if [ $SIZE -eq 0 ]; then
    RED "Aborting: could not download Case distribution"
    exit 1
fi
...
if [ ! "$(command -v $BIN)" = "" ]; then
    ...
    if [ "$(command -v $BIN)" = "" ]; then
        ...
        if [ -f "$ENV_FILE" ]; then
            ...
            echo "  => adding '$TARGET' to 'PATH' variable to execute 'case' from any directory."
...
# sanity check
if [ "$(command -v $BIN)" = "" ]; then
    # installation was completed, but case is not in the PATH
